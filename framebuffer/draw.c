...
{	char	*str;
	...
	cp->x = eval(cp->raw_args[1]);
	cp->y = eval(cp->raw_args[2]);
	cp->radius = eval(cp->raw_args[3]);
	cp->radius = MIN(2 * swidth, cp->radius);
	cp->rgb = eval(cp->raw_args[4]);
